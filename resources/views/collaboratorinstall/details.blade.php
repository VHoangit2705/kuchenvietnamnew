...
<script>
    ...
    $(document).ready(function() {
        ...
        CollaboratorInstallDetails.init({
            ...
            routes: {
                update: "{{ route('dieuphoi.update') }}",
                ctvUpdate: "{{ route('ctv.update') }}",
                agencyUpdate: "{{ route('agency.update') }}",
                updateAddress: "{{ route('dieuphoi.update.address') }}",
                ctvSwitch: "{{ route('ctv.switch') }}",
                ctvClear: "{{ route('ctv.clear') }}",
                collaboratorShow: "{{ route('collaborator.show', ':id') }}",
                getDistrict: "{{ route('ctv.getdistrict', ':province_id') }}",
                getWard: "{{ route('ctv.getward', ':district_id') }}",
                filterCollaborators: "{{ route('collaborators.filter') }}",
                orderHistory: "{{ route('ctv.order.history', ':order_code') }}",
                banksUrl: "{{ config('services.vietqr.banks_url', 'https://api.vietqr.io/v2/banks') }}",
