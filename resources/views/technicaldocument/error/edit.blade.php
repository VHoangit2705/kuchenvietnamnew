            ...
            <div class="card border-0 shadow-sm rounded-4">
                <div class="card-header bg-white border-bottom py-3">
                    ...
                    @if($error->productModel)
                        <small class="text-muted">Model: {{ $error->productModel->model_code }} {{ $error->productModel->version ? '(' . $error->productModel->version . ')' : '' }}</small>
                    @endif
                </div>
                <div class="card-body p-4">
                    ...
                    <form action="{{ route('warranty.document.errors.update', $error->id) }}" method="post">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" name="error_code" value="{{ old('error_code', $error->error_code) }}" required>
                        </div>

                        <div class="mb-3">
                            <label class="form-label fw-semibold">Tên lỗi <span class="text-danger">*</span></label>
                            <input type="text" class="form-control" name="error_name" value="{{ old('error_name', $error->error_name) }}" required>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control" name="description" rows="4">{{ old('description', $error->description) }}</textarea>
                        </div>

                        <div class="d-flex justify-content-end gap-2 mt-4">
                            <a href="{{ route('warranty.document.errors.index', ['model_id' => $error->model_id]) }}" class="btn btn-light border">Hủy bỏ</a>
                            <button type="submit" class="btn btn-primary px-4"><i class="bi bi-save me-1"></i>Cập nhật</button>
                        </div>
                    </form>
                </div>
            </div>
