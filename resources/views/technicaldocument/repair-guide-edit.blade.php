...
@section('content')
                ...
                <div class="d-flex justify-content-between align-items-center bg-white p-4 rounded-4 shadow-sm">
                    <div>
                        ...
                        <p class="text-muted mb-0 small">
                            Mã lỗi: <strong>{{ $guide->commonError->error_code ?? '' }}</strong> —
                            {{ $guide->commonError->error_name ?? '' }}
                            @if($guide->commonError->product)
                                · Sản phẩm: {{ $guide->commonError->product->product_name ?? '' }} — Xuất xứ:
                                {{ $guide->commonError->xuat_xu }}
                            @endif
                        </p>
                    </div>
            ...
            <div class="col-lg-8">
                <div class="card border-0 shadow-sm rounded-4 overflow-hidden">
                    ...
                    <div class="card-body p-4">
                        ...
                        @endif
                        <form action="{{ route('warranty.document.repairGuide.update', $guide->id) }}" method="post"
                            id="formEditGuide">
                            @csrf
                            @method('PUT')
                            <div class="mb-3">
                            ...
                            <div class="d-flex gap-2">
                                <button type="submit" class="btn btn-primary px-4"><i class="bi bi-check-lg me-1"></i>Cập
                                    nhật</button>
                                <button type="button" class="btn btn-outline-danger" id="btnDeleteGuide"><i
                                        class="bi bi-trash me-1"></i>Xóa hướng dẫn</button>
                            </div>
                        </form>
                    </div>
                </div>
                                ...
                                <li
                                    ...
                                    <div class="small">
                                        @if($ver && $ver->file_path)
                                            <a href="{{ $storageUrl . '/' . ltrim($ver->file_path, '/') }}" target="_blank"
                                    ...
                                    <button type="button" class="btn btn-sm btn-outline-danger btn-detach-doc"
                                        data-document-id="{{ $doc->id }}" title="Gỡ tài liệu">
    ...
    <form id="formDeleteGuide" action="{{ route('warranty.document.repairGuide.destroy', $guide->id) }}" method="post"
        class="d-none">
        @csrf
        @method('DELETE')
    </form>
