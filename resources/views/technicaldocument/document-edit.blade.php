...
@section('content')
    ...
    <div class="container-fluid py-5 bg-light">
        ...
        <form action="{{ route('warranty.document.documents.update', $document->id) }}" method="post"
                ...
                <div class="col-lg-5 mb-4">
                    <div class="card border-0 shadow-sm rounded-4 h-100">
                        <div class="card-header bg-white border-bottom py-3">
                            <h5 class="fw-bold mb-0 text-primary"><i class="bi bi-info-circle me-2"></i>Thông tin tài liệu
                            </h5>
                        </div>
                        <div class="card-body p-4">
                            ...
                            <div class="mb-3">
                                ...
                                <textarea class="form-control" name="description" id="docDescriptionEdit"
                                    rows="4">{{ old('description', $document->description) }}</textarea>
                            </div>


                            <div class="d-grid mt-4">
                                <button type="submit" class="btn btn-primary py-2 rounded-pill fw-bold"><i
                                        class="bi bi-check-lg me-1"></i>Lưu Thay Đổi</button>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-lg-7">
                    ...
                    <div class="card border-0 shadow-sm rounded-4 mb-4">
                        <div
                            ...
                            <h5 class="fw-bold mb-0 text-success"><i class="bi bi-file-earmark-check me-2"></i>File Chính
                                (Có version)</h5>
                                ...
                                @foreach($document->documentVersions->sortByDesc('id') as $ver)
                                    <li class="list-group-item d-flex justify-content-between align-items-center py-2">
                                        ...
                                        <div>
                                            ...
                                            <span class="small text-muted">{{ $ext ? strtoupper($ext) : '—' }} •
                                                {{ $ver->created_at->format('d/m/Y H:i') }}</span>
                                        </div>
                                        @if($filePath)
                                            <a href="{{ $storageUrl . '/' . ltrim($filePath, '/') }}" target="_blank"
                        ...
                        <div class="card-body p-4">
                            <div class="row mb-3">
                                <div class="col-12 mb-2">
                                    <label class="form-label fw-semibold small text-primary"><i
                                            class="bi bi-image me-1"></i>Thêm Hình Ảnh</label>
                                    <input type="file" class="form-control" name="attachments_image[]" multiple
                                        accept="image/*">
                                </div>
                                <div class="col-12 mb-2">
                                    <label class="form-label fw-semibold small text-danger"><i
                                            class="bi bi-file-earmark-pdf me-1"></i>Thêm Tài Liệu PDF</label>
                                    <input type="file" class="form-control" name="attachments_pdf[]" multiple accept=".pdf">
                                </div>
                                <div class="col-12 mb-2">
                                    <label class="form-label fw-semibold small text-success"><i
                                            class="bi bi-camera-video me-1"></i>Thêm Video Hướng Dẫn</label>
                                    <input type="file" class="form-control" name="attachments_video[]" multiple
                                        accept="video/*">
                                </div>
                            </div>
                                ...
                                @foreach($document->attachments as $att)
                                    <div class="col-md-4 col-sm-6">
                                        <div class="card h-100 border shadow-sm position-relative group-hover">
                                            ...
                                            <div class="card-body p-2 text-center d-flex flex-column justify-content-center align-items-center"
                                                style="height: 120px; background: #f8f9fa;">
                                                @if(in_array($att->file_type, ['image', 'jpg', 'png', 'jpeg']))
                                                    <img src="{{ $storageUrl . '/' . ltrim($att->file_path, '/') }}"
                                                ...
                                                <a href="{{ $storageUrl . '/' . ltrim($att->file_path, '/') }}" target="_blank"
                                                    class="small text-decoration-none text-dark mt-2 text-truncate w-100 d-block"
                                                    title="{{ $att->file_name }}">
                                                    {{ $att->file_name ?? 'File' }}
                                                </a>
                                            </div>
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ClassicEditor
            .create(document.querySelector('#docDescriptionEdit'), {
