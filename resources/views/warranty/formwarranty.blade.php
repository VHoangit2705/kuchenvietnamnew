...
@section('content')
    ...
    <link rel="stylesheet" href="{{ asset('css/warranty/formwarranty.css') }}">
    <script>
        // Pass data to JavaScript
        window.productList = @json($lstproduct ?? []);
        window.productListAll = @json($products ?? []);
        window.csrfToken = '{{ csrf_token() }}';
        window.warrantyCreateRoute = @json(route('warranty.createwarranty'));
        window.warrantyTakePhotoRoute = @json(route('warranty.takephoto'));
        window.warrantyFindOldRoute = @json(route('warranty.findold'));
        window.getCollaboratorRoute = @json(route('getcollaborator'));
        window.getDistrictRoute = @json(route('ctv.getdistrict', [':province_id']));
        window.getWardRoute = @json(route('ctv.getward', [':district_id']));
    </script>
    <script src="{{ asset('js/validate_input/formwarranty.js') }}"></script>
    <script src="{{ asset('js/warranty/formwarranty-helper.js') }}"></script>
    <script src="{{ asset('js/warranty/formwarranty.js') }}"></script>
    <script src="{{ asset('js/warranty/formwarranty-product.js') }}"></script>
