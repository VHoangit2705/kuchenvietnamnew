                ...
                <table class="table table-striped">
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ $purchasedProducts->groupBy(fn($i) => $i->order_code1 ?? $i->order_code2)->count() }}</td>
                        </tr>
                        <tr>
                    ...
                    @foreach($repairHistory as $item)
                        <div class="timeline-item">
                            <div class="timeline-header d-flex justify-content-between align-items-center">
                                <p>
                                    ...
                                    @else
                                        N/A
                                    @endif
                                    - <strong>Tên sản phẩm:</strong> {{ $item->product_name ?? ($item->warrantyRequest->product ?? 'N/A') }}
                                    @if(isset($item->serial_number) || isset($item->warrantyRequest->serial_number))
                                        - <strong>Mã bảo hành:</strong> {{ $item->serial_number ?? $item->warrantyRequest->serial_number }}
                                    @endif
                                </p>
                            ...
                            <div class="timeline-details" style="overflow: hidden; height: 0;">
                                ...
                                <p><strong>Người tiếp nhận:</strong> {{ $item->staff_received ?? ($item->warrantyRequest->staff_received ?? 'N/A') }}</p>
                            </div>
                        </div>
                    @endforeach
                @else
