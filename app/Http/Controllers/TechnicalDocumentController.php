...
class TechnicalDocumentController extends Controller
{
    ...
    public function downloadAllDocuments(Request $request)
    {
        ...
        foreach ($error->repairGuides as $guide) {
            foreach ($guide->technicalDocuments as $doc) {
                $version = $doc->documentVersions->sortByDesc('id')->first();
                $path = $version->img_upload ?? $version->video_upload ?? $version->pdf_upload;
                if ($version && $path) {
        ...
        if (!empty($uploadedFiles)) {
            ...
            foreach ($uploadedFiles as $index => $file) {
                ...
                if (in_array($ext, ['jpg', 'jpeg', 'png'])) {
                    $documentVersionData['img_upload'] = $path;
                } elseif (in_array($ext, ['mp4', 'webm'])) {
                    $documentVersionData['video_upload'] = $path;
                } elseif ($ext === 'pdf') {
                    $documentVersionData['pdf_upload'] = $path;
                }

                DocumentVersion::create($documentVersionData);
    ...
    public function storeDocument(Request $request)
    {
        ...
        $ext = strtolower($file->getClientOriginalExtension());
        // Chọn thư mục lưu: videos/ cho video, photos/ cho các file khác
        $basePath = in_array($ext, ['mp4', 'webm']) ? 'videos' : 'photos';
